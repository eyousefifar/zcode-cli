...
- [ ] 3.4 Workflow-subsystem e2e (RESIDUAL, documented): the vendor's Workflow tool is NOT registered headless ("Tool not found" verified) and the meta-evaluation/runner helpers are only exercised at the replay layer (test/unit/eval-replay.test.ts). A full workflow run (meta schema validation, request/response protocol, cancellation through the workflow subsystem) needs either a registered Workflow tool in some surface or the app-server adapter — blocked on the same decision as D22 follow-up.

### R4 — P2: docs re-baseline (rides along with R1/R2)
- [x] 4.1 Done: TROUBLESHOOTING TUI section rewritten (false "no TUI in bundle" claim removed); stale `src/bin.js` refs → `src/npm-entry.ts`; README gains checksum-verifying install, credits (kingsword09 + pi), engineering-doc links, non-goals (Windows/musl/Chrome); ENV.md gains `ZCODE_DISABLE_UPDATE_CHECK`, `ZAI_BUSINESS_BASE_URL` (+ D23 caveat), isolation contract, native-addon disclosure, tool-resolution order. (M6, G6)
