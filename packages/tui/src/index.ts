...
import { missingCodingPlanKey } from "./cli/prompt-preflight.ts";
...
class ZCodeTui {
  ...
  async run(): Promise<void> {
    // [fork] an injected terminal (options.terminal) bypasses the TTY gate.
    if (!this.options.terminal && (!process.stdin.isTTY || !process.stdout.isTTY)) {
  ...
  private createTui(mode: TuiMode): TUI {
    ...
    // [fork] honor an injected terminal instead of the process streams.
    const terminal = this.options.terminal ?? new NotifyingProcessTerminal((data) => {
